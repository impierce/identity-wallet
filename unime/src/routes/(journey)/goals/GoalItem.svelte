...
<button
  ...
  {completed ? 'border border-slate-200 border-opacity-100 opacity-50 dark:border-slate-600' : 'dark:bg-dark bg-white'}"
  use:melt={trigger}
>
  <div class="flex items-center">
    <svelte:component this={icons[icon] || icons['Trophy']} class="text-primary mr-4 h-5 w-5" />
    <p
  ...
  {#if completed}
    <CheckCircleRegularIcon class="text-primary h-6 w-6" />
  {:else}
