<script lang="ts">
  ...
  // TODO make it string and fix HTML.
  let icon: any;
  let title: string;
  ...
  $: {
    if ($state?.credentials) {
      credential = $state.credentials.find((c) => $page.params.id === c.id)!!;
      ...
      qrCodeText = JSON.stringify(credential, null, 0);

      entries = { ...credential.data.credentialSubject.achievement };
      hiddenStandardFields.concat(hiddenCustomFields).forEach((key) => delete entries[key]);

      connectionId = determineConnectionId();
  ...
  <div class="content-height relative flex w-full flex-col">
    ...
    <div class="hide-scrollbar grow overflow-y-scroll bg-white dark:bg-dark">
      ...
      <div class="relative flex flex-col overflow-hidden bg-silver px-[15px] py-[20px] dark:bg-navy">
        ...
        <div class="z-10 flex flex-col items-center pt-[15px]">
          <p
            class="line-clamp-2 text-center text-[22px]/[30px] font-semibold tracking-tight text-black dark:text-white"
          >
    ...
    <BottomDrawer>
      ...
      <span slot="content" class="flex flex-col items-center justify-center">
        <!-- Logo -->
        <div
          class="flex h-[75px] w-[75px] flex-col items-center justify-center rounded-[20px] border-[5px] border-white"
        >
