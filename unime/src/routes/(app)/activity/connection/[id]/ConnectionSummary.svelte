...
<div class="flex flex-col items-center justify-center space-y-4">
  <div class="flex w-full flex-col items-center justify-center space-y-4 py-6">
    ...
    <div class="dark:text-grey text-center text-2xl font-semibold text-slate-700">
  ...
  <!-- Details -->
  <div
    class="dark:bg-dark w-full divide-y divide-solid divide-slate-200 rounded-xl border border-slate-200 bg-white dark:divide-slate-600 dark:border-slate-600"
  >
    {#each Object.entries(summary) as entry}
      <div class="flex flex-col items-start px-4 py-[10px]">
        ...
        <p class="dark:text-grey break-all text-[13px]/[24px] font-medium text-slate-800">
          {entry[1]}
        </p>
      </div>
    {/each}
    ...
    {#if $state.dev_mode !== 'Off'}
      <div class="flex flex-col items-start px-4 py-[10px]">
        ...
        <p class="dark:text-grey select-text break-all font-mono text-[13px]/[24px] font-medium text-slate-800">
          {connection.did ?? '-'}
        </p>
        {#if explorerLink}
          <div class="flex w-full justify-center pt-2">
            <button
              type="button"
              class="text-primary h-10 rounded-lg px-4 text-[13px]/[24px] font-medium"
