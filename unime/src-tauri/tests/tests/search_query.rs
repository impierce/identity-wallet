...
// #[tokio::test]
// #[serial_test::serial]
// async fn test_credential_search_query_twice() {
//     setup_state_file();
//     setup_stronghold();
...
#[tokio::test]
#[serial_test::serial]
async fn test_connections_delete_recent() {
    setup_state_file();
    setup_stronghold();
    ...
    let expected_state_2 =
        json_example::<AppState>("tests/fixtures/states/three_connections_search_delete_recent.json");
    assert_state_update(
        AppStateContainer(Mutex::new(state)),
        vec![action],
        vec![Some(expected_state_2)],
