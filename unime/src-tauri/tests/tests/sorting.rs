...
async fn test_connections_sorting_unicode_name_az_reverse() {
    ...
    let state = json_example::<AppState>("tests/fixtures/states/three_connections_unicode.json");
    let action = json_example::<Action>("tests/fixtures/actions/connection_sort_name_az_reverse.json");
    let expected_state =
        json_example::<AppState>("tests/fixtures/states/three_connections_sort_unicode_name_az_reverse.json");
    assert_state_update(
        AppStateContainer(Mutex::new(state)),
        vec![action],
        vec![Some(expected_state)],
