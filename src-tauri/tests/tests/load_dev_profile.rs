...
async fn test_load_dev_profile() {
    ...
    let state = json_example::<AppState>("tests/fixtures/states/pf_with_driverl_and_personal_info_redirect_me.json");
...
async fn test_load_dev_profile_twice() {
    ...
    // Deserializing the Transferstates and Actions from the accompanying json files.
    let state1 = json_example::<AppState>("tests/fixtures/states/pf_with_driverl_and_personal_info_redirect_me.json");
    let state2 = json_example::<AppState>("tests/fixtures/states/pf_with_driverl_and_personal_info_redirect_me.json");
    let action = json_example::<Action>("tests/fixtures/actions/dev_load_profile.json");
    assert_state_update(
        AppState::default(),
        vec![
